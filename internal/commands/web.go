...
import (
	...
	"github.com/leminhson2398/todo-api/internal/route"
	...
	cc := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for i := 0; i < maxRetryNumber; i++ {
				...
				}).WithError(err).Error("issue connection to database, retrying")
			...
			log.WithFields(log.Fields{
				"url": viper.GetString("server.hostname"),
			...
			r, err := route.NewRouter(db, []byte(secret))
			if err != nil {
				log.Errorf("Error creating new router %v", err)
				return err
			}
